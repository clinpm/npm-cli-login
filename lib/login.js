var RegClient = require('@clinpm/npm-registry-client');
...
module.exports = {
    ...
    generateFileContents: function (args, contents, response) {
        // 适配私有镜像路径
        const reg = /(https:\/\/packages\.aliyun\.com\/[\d\w]+\/npm)\/npm-registry/
        if (reg.test(args.registry)) {
            const r = reg.exec(args.registry) || []
            args.registry = r[1]
        }
